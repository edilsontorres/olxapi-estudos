...
module.exports = {
    ...
    addAction: async(req,res)=>{
        ...
        if(!title || !cat) {
            res.json({error: 'Titulo e/ou categorias não foram preenchidos'});
            return;
        }
    ...
    editAction: async(req,res)=>{
        let { id } = req.params;
        let { title, price, priceneg, images, status, desc, cat, token } = req.body;
        ...
        if(cat){
            const category = await Category.findById({slug: cat}).exec();
            if(!category){
                res.json({error: 'Categoria inexistente'});
                return;
            }
